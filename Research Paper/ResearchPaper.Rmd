...
author:
  - name: Xinyue Bai
    #email:  xinyue.bai.2020@mitb.smu.edu.sg
    affiliation: Singapore Management University
  - name: Hongting Li
    #email: hongting.li.2019@mitb.smu.edu.sg
    affiliation: Singapore Management University
  - name: Weimin Zhang
    #email: wmzhang.2020@mitb.smu.edu.sg
    affiliation: Singapore Management University
...
2.  The cluster analysis aims to discover similarities in respondents based on tools they used and hours they spent on data visualisation; to group data visualisation tools and visualisation activities.
...
In statistics, exploratory data analysis is an approach to analyze data sets to summarize their main characteristics. We are going to use statistical graphics to visualize the results of IEDA. Past works focused on numerical variables and explored different facility provided by the “ggplot” package. There are two main areas to improve.[@William2017,@HADLEY]
...
1.	Itemset is one item or combination of items. K-itemset means an itemset with k items. One instance of itemsets is typically called a “transaction”. 
2.  Frequency percentage of X over total number of transaction records, N. Support (X) = frequency(X)/N.
3.  For a rule A => B, confidence shows percentage in which B occurs when A occurs.
4.  Confidence(A => B) = P(A intersect B) /P(A) = frequency(A,B)/frequency(A).
5.  Lift gives correlation between A and B in the rule A => B. Correlation shows how one item-set A effects the item-set B.  Lift of 1 represents independence between A and B. Larger than 1 means presence of A gives positive effect and less than 1 represent negative effect. 
...
### Parallel Sets – a novel approach to compare multiple item sets information
...
# Design framework and Use Case Demonstration
