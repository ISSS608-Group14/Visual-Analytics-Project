...
survey_tech_tr = read.transactions("ItemList.csv", header=TRUE, format = "basket", cols=1, sep=",")
survey_label <- read_csv("labels.csv")
survey_tech_tr@itemsetInfo <- survey_label

education <- unique(survey_label$education)
experience_max <- max(survey_label$experience)

major <- unique(survey_label$major
)
gender <- unique(survey_label$gender)

country <- unique(survey_label$country)

salary <- unique(survey_label$salary)
    ...
    shiny::fluidPage(
      ...
      fluidRow(
        column(3,
               ...
               selectInput("YearlySalary",
                           "Year of relevant experience",
                           salary#,city#placeholder
                           ),
        ...
        shiny::column(3,
          ...
          shiny::sliderInput("supp", "Minimum Support:", min = 0, max = 1, 
                             value = 0.1 , step = 1/10000, sep =""),
          shiny::sliderInput("conf", "Minimum Confidence:", min = 0, max = 1,
                             value = 0.8 , step =  1/1000, sep = ""), 
          shiny::sliderInput("lift", "Minimum Lift:", min = 0, max = 1,
                             value = 0.9 , step =  (1)/1000, sep = ""), 
                                             ...
                                             shiny::wellPanel(
                                               shiny::fluidRow(
                                                 ...
                                                 shiny::column(3, shiny::sliderInput("max_scatter", "Top rules shown (keep below 500):", 
                                                                                     min = 1, max = 500, value = min(100, 500), step = 1, sep = ""))
                                               )),
...
server = function(input, output, session) {
      ...
      survey_tech_tr1 = eventReactive(input$go,{
        survey_tech_tr = subset(survey_tech_tr, subset = survey_tech_tr@itemsetInfo$education == input$education)
        survey_tech_tr = subset(survey_tech_tr, subset = survey_tech_tr@itemsetInfo$exprience <= input$experience)
        survey_tech_tr = subset(survey_tech_tr, subset = survey_tech_tr@itemsetInfo$major == input$major)
        survey_tech_tr = subset(survey_tech_tr, subset = survey_tech_tr@itemsetInfo$gender == input$gender)
        survey_tech_tr = subset(survey_tech_tr, subset = survey_tech_tr@itemsetInfo$country == input$country)
        survey_tech_tr = subset(survey_tech_tr, subset = survey_tech_tr@itemsetInfo$salary == input$YearlySalary)
        survey_tech_tr
        
      })
      

      all_survey_tech_ft = apriori(survey_tech_tr1(),
                                   parameter=list(
                                     support =0.1,
                                     conf=0.8, minlen=2),
      ...
      dataset <- x
      aparameter <- as(NULL,'APparameter')
